...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   ! -----------------------------
   character(CL) :: hist_a2x3hr_flds  = 'Sa_z:Sa_u:Sa_v:Sa_tbot:Sa_ptem:Sa_shum:Sa_dens:Sa_pbot:Sa_pslv:Faxa_lwdn:Sa_obk'
   ! end
...
!     !        call mct_aVect_accumulate(x2o_ox(eoi), x2oacc_ox(eoi))
         ...
         do exi = 1,num_inst_xao
            ...
            if (trim(aoflux_grid) == 'ocn') then
               ...
               call seq_flux_atmocn_mct( cdata_ox, a2x_ox(eai),o2x_ox(eoi),i2x_ox(eoi),xao_ox(exi))
               call seq_flux_ocnalb_mct(cdata_ox,xao_ox(exi),fractions_ox(efi))
      ...
      if (ocn_present .and. iamin_CPLID) then
         ...
         if (trim(aoflux_grid) == 'atm') then
            ...
            do exi = 1,num_inst_xao 
               ...
               ! Modified by Songyj
               ! add i2x_ax
               call seq_flux_atmocn_mct( cdata_ax, a2x_ax(eai), o2x_ax(eoi),i2x_ax(eoi), xao_ax(exi))
               call seq_flux_ocnalb_mct(cdata_ox,xao_ox(exi),fractions_ox(efi))
