module seq_flux_mct
  ...
  use shr_flux_mod,      only: shr_flux_atmocn, shr_flux_spray
...
! -----------------------------
! Modified by Songyj 2018-12-18
! -----------------------------
  real(r8), allocatable ::  obklen(:)  ! atm obklen 
  real(r8), allocatable ::  socn  (:)  ! ocean sss
  real(r8), allocatable ::  uu10n(:)   ! diagnostic: 10m wind speed 
  real(r8), allocatable ::  uv10n(:)   ! diagnostic: 10m wind speed 
  real(r8), allocatable ::  hs(:)      ! ocean wave height
  real(r8), allocatable ::  hs_sp(:)   ! spray sensible heat
  real(r8), allocatable ::  hl_sp(:)   ! spray latent heat
  real(r8), allocatable ::  tau_sp(:)  ! spray momentum flux
  real(r8), allocatable ::  iFrac(:)   ! ice fraction
! end Modify

!... added by BaoYing, 20190103
  real(r8), allocatable ::  ustokes(:) ! stokes velocity, zonal
  real(r8), allocatable ::  vstokes(:) ! stokes velocity, meridional
!... end add 	
!------------------------------
  ...
  ! Modified by Songyj 2018-12-18
  ! -----------------------------
  integer :: index_a2x_Sa_obk  
  integer :: index_o2x_So_s
  integer :: index_o2x_So_hs
  integer :: index_i2x_Si_ifrac
  integer :: index_xao_Faox_hssp
  integer :: index_xao_Faox_hlsp
  ! end Modify
  !------------------------------

!... added by BaoYing, 20190103
  integer :: index_o2x_So_us     ! stokes velocity, zonal
  integer :: index_o2x_So_vs     ! stokes velocity, meridional
!... end add 	
...
!===============================================================================
  ...
  subroutine seq_flux_init_mct( cdata, fractions)
    ...
    ! Modified by Songyj 2018-12-18
    ! -----------------------------
    allocate( obklen(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate obklen',ier)
    
    
    allocate( socn(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate socn',ier)
...
!... added by BaoYing, 20190103
    allocate( ustokes(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate ustokes',ier)
    allocate( vstokes(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate vstokes',ier)
!... end add 	
    ...
    ! Modified by Songyj 2018-12-18
    ! -----------------------------
    allocate( uu10n(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate uu10n',ier)

    allocate( uv10n(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate uv10n',ier)

    allocate( hs_sp(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate hs_sp',ier)

    allocate( hl_sp(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate hl_sp',ier)
    
    allocate( tau_sp(nloc),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate tau_sp',ier)
...
!... added by BaoYing, 20190103
    allocate( ustokes(nloc_a2o),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate ustokes',ier)
    allocate( vstokes(nloc_a2o),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate vstokes',ier)
!... end add 	
...
!... added by BaoYing, 20190103
          ustokes(n) =0.0_R8 ! stokes velocity, zonal        ~ m/s
          vstokes(n) =0.0_R8 ! stokes velocity, meridional   ~ m/s
!... end add 	
      enddo
    else        
...
!... added by BaoYing, 20190103
          ustokes(n) =o2x_e%rAttr(index_o2x_So_us   ,io)
          vstokes(n) =o2x_e%rAttr(index_o2x_So_vs   ,io)
!... end add 	
       enddo
...
!                          tocn , emask, sen , lat , lwup , &
                          tocn ,ustokes, vstokes, emask, sen , lat , lwup , &
!... end mod
...
!===============================================================================
  ...
  subroutine seq_flux_atmocn_mct( cdata, a2x, o2x, i2x, xao)
    ...
    if (first_call) then
       ...
       ! Modified by Songyj 2018-12-18
       ! -----------------------------
       index_a2x_Sa_obk   = mct_aVect_indexRA(a2x,'Sa_obk')
       index_o2x_So_s     = mct_aVect_indexRA(o2x,'So_s')
       index_o2x_So_hs    = mct_aVect_indexRA(o2x,'So_hs')
       index_i2x_Si_ifrac = mct_aVect_indexRA(i2x,'Si_ifrac')
        

       index_xao_Faox_hssp  = mct_aVect_indexRA(xao,'Faox_hssp')
       index_xao_Faox_hlsp  = mct_aVect_indexRA(xao,'Faox_hlsp')
 ...
 !... added by BaoYing, 20190103
       index_o2x_So_us      = mct_aVect_indexRA(o2x,'So_ustokes')
       index_o2x_So_vs      = mct_aVect_indexRA(o2x,'So_vstokes')
!... end add 	
...
!... end add 	
          ...
          ! Modified by Songyj 2018-12-18
          ! -----------------------------
          obklen(n) = 0.0_R8 ! atm obklen
          socn(n)   = 0.0_R8 ! ocn sss
          hs(n)     = 0.0_R8 ! ocn wave height
       enddo
    else	
       do n = 1,nloc
          if (mask(n) /= 0) then	
...
!... added by BaoYing, 20190103
             ustokes(n) = o2x%rAttr(index_o2x_So_us   ,n)
             vstokes(n) = o2x%rAttr(index_o2x_So_vs   ,n)
!... end add 	
             ! -----------------------------
             ! Modified by Songyj 2018-12-18
             ! -----------------------------
             obklen(n)= a2x%rAttr(index_a2x_Sa_obk,n)
             socn(n)  = o2x%rAttr(index_o2x_So_s  ,n)
             hs(n)    = o2x%rAttr(index_o2x_So_hs ,n)
             iFrac(n) = i2x%rAttr(index_i2x_Si_ifrac ,n)
...
!                         duu10n,ustar, re  , ssq, missval = 0.0_r8 )
                     ...
                     ! Modified by Songyj 2018-12-18
                     ! -----------------------------
                          duu10n, uu10n,uv10n,ustar, re, ssq)


    !write(logunit,*) 'songyj spray ifrac', maxval(iFrac), minval(iFrac)
    !write(logunit,*) 'songyj spray mask', maxval(mask), minval(mask)
    !write(logunit,*) 'songyj spray emask', maxval(emask), minval(emask)
    ! -----------------------------
    ! add spray latent and sensible heat

    ! Modified by Songyj 2018-12-18
    ! -----------------------------

    call shr_flux_spray (nloc , emask, uu10n, tocn, tbot, zbot, &  
                         hs,   shum, socn,  ustar,             &
                         obklen, tau_sp,hs_sp, hl_sp)          

     DO n=1,nloc
        if (emask(n) /= 0 .and. (1-iFrac(n)) > 0.5 )  then
            if (hl_sp(n) .lt. 1000) lat(n) = lat(n) - hl_sp(n)*(1-iFrac(n))
            if (hs_sp(n) .lt. 1000) sen(n) = sen(n) - hs_sp(n)*(1-iFrac(n))
    ...
    !write(logunit,*) 'songyj spray flux hl_sp',hl_sp(1:50)
    ...
    do n = 1,nloc
	  ...
	  xao%rAttr(index_xao_So_qref  ,n) = qref(n)
          ...
          ! Modified by Songyj 2018-12-18
          xao%rAttr(index_xao_Faox_hssp ,n) = hs_sp(n)
          xao%rAttr(index_xao_Faox_hlsp ,n) = hl_sp(n)
          ! end
