...
def plot(model, x_train, y_train, x_test, y_test, multiple, multiple_x):
    ...
    y_test *= multiple
    x_test *= multiple_x
    ax1.plot(x_test[:, 0, 0], y_test, color='b', linestyle='-.', marker='.', label='TrainData')
    ...
    x_train *= multiple_x
    y_train *= multiple
    ax1.plot(x_train[:, 0], y_train[:, 0], color='k', linestyle='-.', marker='.', label='TestData')
...
def run_train(epochs, model, optimizer, scheduler, ntrain, ntest, s):
    ...
    in_x = np.double([1024, 8, 16, 24, 32, 48,  64, 96, 128, 192, 256, 384, 512, 768, 1024])
    in_y = np.double([46626344166, 399199208083, 219055446333, 139703296546, 107357465823, 78512426984, 60428612025, 44844770889, 
                        38071564565, 29848368575, 27760595349,  25502638773, 16876663419,  6421727681, 46626344166])
    # normalize
    mean = np.mean(in_y)
    multiple = 1
    while mean > 10:
