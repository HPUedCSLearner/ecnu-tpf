...
def run_train(epochs, model, optimizer, scheduler, ntrain, ntest, s):
    ...
    in_x = np.double([1024, 8, 16, 24, 32, 48,  64, 96, 128, 192, 256, 1024, 384, 512, 768, 1024])
    in_y = np.double([  194.056, 4097.23, 2381.801, 1497.331, 1180.71, 921.891, 818.266, 605.158, 
                        463.954, 332.224, 265.31,  194.056, 209.239,  183.137, 173.966, 194.056])
    # normalize
    mean = np.mean(in_y)
    multiple = 1
    while mean > 10:
