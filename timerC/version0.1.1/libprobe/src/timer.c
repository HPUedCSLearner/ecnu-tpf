...
void __cyg_profile_func_exit(void *func, void *caller) {
  ...
  if (stackSzie(&funcDddrStk) == 1) { // 如果是main func
    sprintf(callee_caller, "%p<-%p", pop(&funcDddrStk), caller);
  } else {
    unsigned long long callee = pop(&funcDddrStk);
    unsigned long long caller = top(&funcDddrStk);
    sprintf(callee_caller, "%p<-%p", callee, caller);
  }
  ...
  if (hash_has(hash, new_callee_caller) == 0) {
    ...
    sscanf(hash_get(hash, new_callee_caller), "%d:%lld:%lld", &time, &accTime, &shlTIme);
    #ifdef __DEBUG_PRINT__
