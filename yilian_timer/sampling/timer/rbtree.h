...
void inorder(struct node *root,int funcid,FILE *fw)
{
    ...
    fprintf(fw,"%lx\n%lx\n",addr2int[funcid],addr2int[root->data.fatherID]);//,%llu,%llu\n,node.times,node.accTime/2
    // fprintf(fw,", times:%llu, acctime:%llu, alltime:%llu",root->data.times,root->data.accTime/2,root->data.shelltime/2);
    // fprintf(fw,", times:%llu, acctime:%.3fms, alltime:%.3fms",root->data.times,1.0*root->data.accTime/2199998,1.0*root->data.shelltime/2199998);
    fprintf(fw,", %llu, %llu, %llu",root->data.times,10*root->data.accTime/21,10*root->data.shelltime/21);
    fprintf(fw,"\n");
    
    // fprintf(fw,"%llu,%llu,%llu,",root->data.times,root->data.accTime/2,root->data.shelltime/2);
	// fprintf(fw,"\n");
...
int search(struct node *T,int data,int ans,unsigned long b){
    if(T!=NULL){
        // printf("##search---> T->data.fatherID:%ld data:%d ans:%d b=%ld\n",T->data.fatherID,data,ans,b);
        ...
        else
		{
			if(T->data.fatherID==data)
			{
				...
				// asm volatile("rdtsc\n\t": "=a" (eax), "=d" (edx));
				// T->data.shelltime+=(((unsigned long long)eax | (unsigned long long)edx << 32) - func_begin_time);
				return 1;
			}
			else
			{
